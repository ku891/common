...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/ku891/common build/common
...
git clone -b main --depth 1 https://github.com/ku891/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" == "ku891/autobuild" ]]; then
  rm -rf backups
fi
git add .
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="ku891/build-actions"
  sudo rm -rf build common.sh
